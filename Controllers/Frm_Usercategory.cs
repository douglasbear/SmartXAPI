                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    else
                    {
                        if (UserCatID == 0)
                        {
                            ...
                            string sqlCommandPrintTemplates = "insert into Gen_PrintTemplates SELECT N_CompanyID, N_FormID, X_RptName, X_Criteria, " + N_UserCategoryID + " AS N_UserCategoryID, N_PrintCopies, X_RptFolder, X_PkeyField, B_ClearScreenAfterSave, N_Custom, B_Custom FROM Gen_PrintTemplates where N_UserCategoryID=" + FromUserCatID;
