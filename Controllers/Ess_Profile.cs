...
namespace SmartxAPI.Controllers
{
    ...
    public class Ess_Profile : ControllerBase
    {
        ...
        public ActionResult GetProfileDetails(int nEmpID, int nFnyearID)
        {
            ...
            string sqlEmployeeDetails = "SELECT vw_PayEmployee.*, Pay_Employee.X_EmpName AS X_ReportTo FROM Pay_Supervisor LEFT OUTER JOIN Pay_Employee ON Pay_Supervisor.N_EmpID = Pay_Employee.N_EmpID AND Pay_Supervisor.N_CompanyID = Pay_Employee.N_CompanyID RIGHT OUTER JOIN vw_PayEmployee ON Pay_Supervisor.N_SupervisorID = vw_PayEmployee.N_ReportToID AND Pay_Supervisor.N_CompanyID = vw_PayEmployee.N_CompanyID AND Pay_Employee.N_FnYearID = vw_PayEmployee.N_FnYearID where vw_PayEmployee.N_CompanyID=@p1 and vw_PayEmployee.N_FnYearID=@p2 and vw_PayEmployee.N_EmpID=@p3";
