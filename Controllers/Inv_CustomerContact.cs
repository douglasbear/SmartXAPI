...
namespace SmartxAPI.Controllers
{
    ...
    public class Customer_Contact : ControllerBase
    {
        ...
        [HttpGet("details")]
        public ActionResult ContactListDetails(int nContactID)
        {
            DataTable dt = new DataTable();
            SortedList Params = new SortedList();
            int nCompanyId = myFunctions.GetCompanyID(User);

            string sqlCommandText = "select * from vw_Inv_CustomerContact where N_CompanyID=@p1 and N_ContactID=@p2";
            Params.Add("@p1", nCompanyId);
            Params.Add("@p2", nContactID);
