    ...
    public class Pay_Appraisalform : ControllerBase
    {
        ...
        public ActionResult GetAppraisalList(int? nCompanyId, int nFnYearID, int nType, int nPage, int nSizeperpage, string xSearchkey,string xUserCategory,string xSortBy,int nUserID)
        {
          ...
          try
           ...
           // X_UserCategory = dLayer.ExecuteScalar("Select X_UserCategoryList from Sec_User where N_CompanyID=@nCompanyID and N_UserID=@nUserID " +  sqlCommandText, Params, connection).ToString();
