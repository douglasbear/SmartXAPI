    ...
    public class CRM_Customer : ControllerBase
    {
        ...
        public ActionResult CustomerList(int nPage,int nSizeperpage, string xSearchkey, string xSortBy)
        {
            ...
            if (xSortBy == null || xSortBy.Trim() == "")
             ...
             if(Count==0)
                sqlCommandText = "select top("+ nSizeperpage +") * from vw_CRMCustomer where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 " + Searchkey + " " + xSortBy;
            else
                sqlCommandText = "select top("+ nSizeperpage +") * from vw_CRMCustomer where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 " + Searchkey + " and N_CustomerID not in (select top("+ Count +") N_CustomerID from vw_CRMCustomer where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 " + xSortBy + " ) " + xSortBy;
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    sqlCommandCount = "select count(*) as N_Count  from vw_CRMCustomer where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 ";
