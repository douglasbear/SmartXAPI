...
namespace SmartxAPI.Controllers
{
    ...
    public class CRM_Activity : ControllerBase
    {
        ...
        public ActionResult ActivityList(int nPage, int nSizeperpage, string xSearchkey, string xSortBy, bool bySalesMan)
        {
            ...
            if (Count == 0)
                sqlCommandText = "select top(" + nSizeperpage + ") * from vw_CRM_Activity where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 " + Searchkey + Criteria + xSortBy;
            else
                sqlCommandText = "select top(" + nSizeperpage + ") * from vw_CRM_Activity where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 " + Searchkey + Criteria + " and N_ActivityID not in (select top(" + Count + ") N_ActivityID from vw_CRM_Activity where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 " + Criteria + xSortBy + " ) " + xSortBy;
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    sqlCommandCount = "select count(*) as N_Count  from vw_CRM_Activity where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 " + Searchkey + Criteria;
