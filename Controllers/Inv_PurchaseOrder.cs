            ...
            try
            {
                ...
                // }
                    ...
                    if (screen == "DelayedOrder")
                        criteria = "and MONTH(Cast(D_POrderDate as DateTime)) = MONTH(CURRENT_TIMESTAMP) and YEAR(D_POrderDate)= YEAR(CURRENT_TIMESTAMP) and ISNULL(N_Processed,0)=0";
                    ...
                    if (Count == 0)
                        sqlCommandText = "select  top(" + nSizeperpage + ") [order No],vendor,[order Date],x_Description,x_ProjectName,n_Amount,N_POrderID,B_IsSaveDraft,N_ClosingStatusID,X_StatusDescription,X_TypeName from vw_InvPurchaseOrderNo_Search_Cloud where N_CompanyID=@p1 and N_FnYearID=@p2 "  + criteria+ Searchkey + " " + xSortBy;
                    else
