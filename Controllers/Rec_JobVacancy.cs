        ...
        public ActionResult SaveData([FromBody] DataSet ds)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    {
                        N_NextApproverID = myFunctions.LogApprovals(Approvals,myFunctions.getIntVAL(nFnYearID.ToString()), "JOB VACANCY", nVacancyID, MasterTable.Rows[0]["X_VacancyCode"].ToString(), 1,"",0, "",myFunctions.getIntVAL(objUserID.ToString()), User, dLayer, connection, transaction);
