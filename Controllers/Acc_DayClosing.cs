...
namespace SmartxAPI.Controllers
{
    ...
    public class Acc_DayClosing : ControllerBase
    {
        ...
        public ActionResult GetCashSummaryDetails(int nCompanyID,int nFnYearID,int nBranchID,DateTime dTransDate)
        {
            ...
            string sqlCommandText="Select N_CompanyID,SUM(N_AmountDr)as N_AmountDr,SUM(N_AmountCr) as N_AmountCr,D_TransDate,N_BranchID,N_FnYearID,X_CustomerName,N_Order from  vw_DailySummaryPOS where N_CompanyID=@p1 and N_FnYearID=@p2 and N_BranchID=@p3 and cast(D_TransDate as date)=@p4 group by N_CompanyID,D_TransDate,N_BranchID,N_FnYearID,X_CustomerName,n_order order by n_order";
            string sqlCommandTotal="Select SUM(N_AmountDr)as N_Amount from  Vw_DailySalesSummaryPOS where  cast(D_TransDate as date)=@p4 group by N_CompanyID,D_TransDate,N_BranchID,N_FnYearID";
        ...
        public ActionResult GetSalesSummaryDetails(int nCompanyID,int nFnYearID,int nSessionID,DateTime dTransDate)
        {
            ...
            Params.Add("@p4",dTransDate);
            string sqlCommandText="Select * from Vw_POSSessionSummery where N_CompanyID=@p1 and N_FnYearID=@p2 and N_SessionID=@p3 and cast(D_TransDate as date)=@p4 ";
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
