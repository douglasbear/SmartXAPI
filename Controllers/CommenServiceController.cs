        ...
        public ActionResult AuthenticateCustomer(string reqType, int appID, string customerKey)
        {
            try
            {
                ...
                if (reqType.ToLower() == "vendor")
                {
                    ...
                    {
                        conn.Open();
                        string sql = "SELECT Acc_Company.N_CompanyID, Acc_Company.X_CompanyName, Sec_User.X_UserID, Sec_User.N_UserID, Acc_Company.N_ClientID FROM Inv_Customer LEFT OUTER JOIN Acc_Company ON Inv_Customer.N_CompanyID = Acc_Company.N_CompanyID RIGHT OUTER JOIN Sec_User ON Sec_User.N_CompanyID = Inv_Customer.N_CompanyID AND Sec_User.N_CustomerID = Inv_Customer.N_CustomerID WHERE Sec_User.N_LoginFlag="+appID+" Inv_Customer.N_CustomerID= " + nVendorID;
                        SortedList Params = new SortedList();
                        DataTable output = dLayer.ExecuteDataTable(sql, conn);
                        if (output.Rows.Count == 0)
                        {
