...
namespace SmartxAPI.Controllers
{
    ...
    public class SalesDashboard : ControllerBase
    {
        ...
        public ActionResult GetDashboardDetails(int nFnYearID,int nBranchID,bool bAllBranchData)
        {
            ...
            string sqlCurrentInvoice = "SELECT COUNT(*) as N_ThisMonth,sum(Cast(REPLACE(X_BillAmt,',','') as Numeric(10,2)) ) AS  TotalAmount  FROM vw_InvSalesInvoiceNo_Search_New_Cloud WHERE  N_Hold=0 and MONTH(Cast([Invoice Date] as DateTime)) = MONTH(CURRENT_TIMESTAMP) AND YEAR(Cast([Invoice Date] as DateTime)) = YEAR(CURRENT_TIMESTAMP) and N_FormID = 64  and  X_TransType = 'SALES' and N_SalesType = 0 and  N_CompanyID = " + nCompanyID  + " and N_FnyearID="+nFnYearID + crieteria;
