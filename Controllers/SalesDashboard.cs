            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    else
                    {
                      ...
                      sqlCurrentInvoice = "SELECT count(1) as N_ThisMonth,sum(Cast(REPLACE(TotalAmount,',','') as Numeric(16,2)) ) AS  TotalAmount  FROM Vw_SalesRevenew_Cloud WHERE MONTH(Cast(D_SalesDate as DateTime)) = "+MonthWiseDate+" AND YEAR(Cast(D_SalesDate as DateTime)) = "+YearWiseDate+" and N_CompanyID = " + nCompanyID  + " and N_FnyearID="+nFnYearID + crieteria;
