                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    else
                    {
                     ...
                     //string sqlCurrentInvoice = "SELECT count(1) as N_ThisMonth,CAST(CONVERT(varchar, CAST(sum(Cast(REPLACE(X_BillAmt,',','') as Numeric(10,2)) ) AS Money), 1) AS   varchar) AS  TotalAmount  FROM vw_InvSalesInvoiceNo_Search WHERE MONTH(Cast([Invoice Date] as DateTime)) ="+MonthWiseDate+"AND YEAR(Cast([Invoice Date] as DateTime)) = YEAR(CURRENT_TIMESTAMP) and N_CompanyID = " + nCompanyID  + " and N_FnyearID="+nFnYearID;
                      ...
                      DraftedDelivery="SELECT count(1) as N_Count FROM vw_InvDeliveryNoteNo_Search WHERE B_IsSaveDraft=1 and MONTH(Cast(invoiceDate as DateTime)) = "+MonthWiseDate+" and YEAR(invoiceDate) = "+YearWiseDate+" and N_CompanyID = " + nCompanyID  + " and N_FnyearID="+nFnYearID + crieteria;
                      DraftedInvoice="SELECT count(1) as N_Count FROM vw_InvSalesInvoiceNo_Search_New_Cloud WHERE B_IsSaveDraft=1 and MONTH(Cast(invoiceDate as DateTime)) = "+MonthWiseDate+" and YEAR(invoiceDate) = "+YearWiseDate+" and N_CompanyID = " + nCompanyID  + " and N_FnyearID="+nFnYearID + crieteria;
