...
namespace SmartxAPI.Controllers
{
    ...
    public class SalesDashboard : ControllerBase
    {
        ...
        public ActionResult GetDashboardDetails()
        {
            ...
            string sqlCurrentOrder = "SELECT COUNT(*) as N_ThisMonth,sum(Cast(REPLACE(X_BillAmt,',','') as Numeric(10,2)) ) as TotalAmount FROM vw_InvSalesOrderNo_Search WHERE MONTH(Cast[Invoice Date] as DateTime) = MONTH(CURRENT_TIMESTAMP) AND YEAR(Cast[Invoice Date] as DateTime) = YEAR(CURRENT_TIMESTAMP)";
            string sqlCurrentInvoice = "SELECT COUNT(*) as N_ThisMonth,sum(Cast(REPLACE(N_Amount,',','') as Numeric(10,2)) ) as TotalAmount FROM vw_InvSalesInvoiceNo_Search WHERE MONTH(D_OrderDate) = MONTH(CURRENT_TIMESTAMP) AND YEAR(D_OrderDate) = YEAR(CURRENT_TIMESTAMP)";
