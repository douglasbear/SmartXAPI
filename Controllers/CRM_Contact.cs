...
namespace SmartxAPI.Controllers
{
    ...
    public class CRM_Contact : ControllerBase
    {
        ...
        //          = "and (X_Contact like '%" + xSearchkey + "%'or X_ContactCode like'%" + xSearchkey + "%'or X_Department like'%" + xSearchkey + "%')";
        ...
        //             sqlCommandCount = "select count(1) as N_Count  from vw_CRMContact where N_CompanyID=@p1 "+Searchkey;
        ...
        public ActionResult OpportunityList(int nFnYearId,int nPage, int nSizeperpage, string xSearchkey, string xSortBy)
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    sqlCommandCount = "select count(1) as N_Count  from vw_CRMContact where N_CompanyID=@p1 " + Pattern;
                    object TotalCount = dLayer.ExecuteScalar(sqlCommandCount, Params, connection);
                    OutPut.Add("Details", api.Format(dt));
                    OutPut.Add("TotalCount", TotalCount);
                    if (dt.Rows.Count == 0)
