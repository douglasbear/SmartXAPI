...
namespace SmartxAPI.Controllers
{
    ...
    public class Gen_Notifications : ControllerBase
    {
        ...
        [HttpGet("count")]
        public ActionResult NotificationCount(bool bShowAllBranch, int nBranchID,int nFnYearID)
        {
           ...
           if (nFnYearID>0){
              criteria="and n_fnyearID=@nFnYearID";
            }


            if (bShowAllBranch)
            {
                sqlCommandText = "select count(1) from vw_ApprovalPending where N_CompanyID=@nCompanyID and N_NextApproverID=@nUserID " + criteria + " ";
            }
            else
            {
                sqlCommandText = "select count(1) from vw_ApprovalPending where N_CompanyID=@nCompanyID and N_NextApproverID=@nUserID and N_Branchid = @nBranchID " + criteria +"";
        ...
        public ActionResult GetNotification(int nFnYearID)
        {
            SortedList Params = new SortedList();
