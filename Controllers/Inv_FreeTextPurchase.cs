        ...
        public ActionResult GetPurchaseeDetails(int nCompanyId, int nFnYearId, string xInvoiceNO, string xTransType, bool showAllBranch, int nBranchId)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    " N_VoucherID, N_VoucherDetailsID, N_CostCentreID,X_CostCentreName as X_CostcentreName,N_CostCentreID as n_Segment_2,X_CostCentreCode,N_BranchID,X_BranchName,X_BranchCode , " +
