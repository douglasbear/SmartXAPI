...
namespace SmartxAPI.Controllers
{
    ...
    public class SchDashboard : ControllerBase
    {
        ...
        public ActionResult GetDashboardDetails(int nFnYearID,int nBranchID,bool bAllBranchData)
        {
            ...
            string sqlStudent = "SELECT COUNT(*) as N_Count FROM vw_SchAdmission WHERE N_CompanyID = " + nCompanyID + " and N_FnyearID="+nFnYearID  + crieteria ;
            string sqlMaleStudent = "SELECT COUNT(*) as N_Count FROM vw_SchAdmission WHERE x_Gender='Male' and  N_CompanyID = " + nCompanyID + " and N_FnyearID="+nFnYearID  + crieteria ;
            string sqlFemaleStudent = "SELECT COUNT(*) as N_Count FROM vw_SchAdmission WHERE x_Gender='FeMale' and N_CompanyID = " + nCompanyID + " and N_FnyearID="+nFnYearID  + crieteria ;
            ...
            string sqlCourseBasedStudents = "select X_Class,COUNT(1) as N_Count from  vw_SchAdmission WHERE  N_CompanyID = " + nCompanyID + " and N_FnyearID="+nFnYearID  + crieteria + "  group by X_Class";
            string sqlCountryBasedStudents = "SELECT X_Nationality,COUNT(1) as N_Count from  vw_SchAdmission WHERE  N_CompanyID = " + nCompanyID + " and N_FnyearID="+nFnYearID  + crieteria + "  group by X_Nationality";
