        ...
        public ActionResult list(string date,int nCompanyID,int nFnYearID,int nPage,int nSizeperpage,string xSearchKey, string xSortBy)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (Count == 0){
                        sqlCommandText1 = "select top(" + nSizeperpage + ") * from Vw_customerReceivable where N_DueAmount<>0 and N_CompanyID=N_CompanyID" + Searchkey + xSortBy ;
                    } else {
                        sqlCommandText1 = "select top(" + nSizeperpage + ") * from Vw_customerReceivable where  N_DueAmount<>0 and N_CompanyID=N_CompanyID" + Searchkey + " and X_CustomerCode not in (select top(" + Count + ") X_CustomerCode from Vw_customerReceivable where N_CompanyID=N_CompanyID" + Searchkey + xSortBy + ")" + xSortBy ;
                    }
                    dt = dLayer.ExecuteDataTable(sqlCommandText1, Params, connection, transaction);

                    sqlCommandCount = "select count(1) as N_Count,sum(Cast(REPLACE(N_DueAmount,',','') as Numeric(16,4)) ) as TotalAmount from Vw_customerReceivable where N_DueAmount<>0 and N_CompanyID=N_CompanyID";
        ...
        public ActionResult detailList(string xCustomerCode,string date,int nCompanyID,int nFnYearID,int nPage,int nSizeperpage,string xSearchKey,string xSortBy)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    dLayer.ExecuteDataTablePro("SP_InvReceivablesAsOfDate", Params, connection, transaction);
                    // sqlCommandText = "select * from Vw_customerReceivableDetails where N_CompanyID=N_CompanyID and X_CustomerCode=CAST('"+xCustomerCode+"' as VARCHAR)" + Searchkey + xSortBy ;
                    // dt = dLayer.ExecuteDataTable(sqlCommandText, Params, connection, transaction);

                    if (Count == 0){
                        sqlCommandText1 = "select top(" + nSizeperpage + ") * from Vw_customerReceivableDetails where N_BalanceAmount<>0 and N_CompanyID=N_CompanyID and X_CustomerCode=CAST('"+xCustomerCode+"' as VARCHAR)" + Searchkey + xSortBy ;
                    } else {
                        sqlCommandText1 = "select top(" + nSizeperpage + ") * from Vw_customerReceivableDetails where N_BalanceAmount<>0 and N_CompanyID=N_CompanyID and X_CustomerCode=CAST('"+xCustomerCode+"' as VARCHAR)" + Searchkey + " and X_ReferenceNo not in (select top(" + Count + ") X_ReferenceNo from Vw_customerReceivableDetails where N_CompanyID=N_CompanyID" + Searchkey + xSortBy + ")" + xSortBy;
                    }
                    dt = dLayer.ExecuteDataTable(sqlCommandText1, Params, connection, transaction);

                    sqlCommandCount = "select count(1) as N_Count,sum(Cast(REPLACE(N_BalanceAmount,',','') as Numeric(16,4)) ) as TotalAmount from Vw_customerReceivableDetails where N_BalanceAmount<>0 and N_CompanyID=N_CompanyID and X_CustomerCode=CAST('"+xCustomerCode+"' as VARCHAR)";
                    DataTable Summary = dLayer.ExecuteDataTable(sqlCommandCount, Params, connection, transaction);
                    string TotalCount = "0";
                    string TotalSum = "0";
                    if (Summary.Rows.Count > 0){
