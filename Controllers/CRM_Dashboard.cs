    ...
    public class CrmDashboard : ControllerBase
    {
        ...
        public ActionResult GetDashboardDetails(int nFnYearId)
        {
           ...
           // string sqlOpportunitiesStage = "select X_Stage,CAST(COUNT(*) as varchar(50)) as N_Percentage  from vw_CRMOpportunity group by X_Stage";
            string sqlOpportunitiesStage = "select isNull(X_Stage,'Others') as X_Stage,CAST(COUNT(*) as varchar(50)) as N_Percentage  from vw_CRMOpportunity where N_CompanyID = "+nCompanyID + Pattern +" group by X_Stage,N_Sort order by ISNULL(N_Sort,1000)";
            ...
            string sqlCurrentRevenue = "SELECT COUNT(*) as N_ThisMonth,sum(Cast(REPLACE(N_ExpRevenue,',','') as Numeric(10,2)) ) as TotalAmount FROM vw_CRMOpportunity WHERE MONTH(D_EntryDate) = MONTH(CURRENT_TIMESTAMP) AND YEAR(D_EntryDate) = YEAR(CURRENT_TIMESTAMP) and isnull(N_SalesID,0)<>0 and N_CompanyID = "+nCompanyID+""+ Pattern;
