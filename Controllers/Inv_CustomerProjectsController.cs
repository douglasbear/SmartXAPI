...
namespace SmartxAPI.Controllers
{
  ...
  [HttpGet("details")]
        ...
        public ActionResult ProjectList(int nPage, int nSizeperpage, string xSearchkey, string xSortBy,int nEmpID)
        {
            ...
            if (Count == 0)
                sqlCommandText = "select top(" + nSizeperpage + ") X_ProjectCode,X_ProjectName,X_CustomerName,X_District,X_Name,N_StatusID,D_StartDate,N_ContractAmt,N_EstimateCost,AwardedBudget,ActualBudget,CommittedBudget,RemainingBudget,X_PO,N_Progress,N_CompanyID,N_Branchid,N_CustomerID,B_IsSaveDraft,B_Inactive,N_ProjectID,N_StageID,X_Stage,CONVERT(VARCHAR(10), D_EndDate,111) as D_EndDate,N_LastActionID,X_ClosingRemarks,x_TaskSummery from vw_InvProjectDashBoard_Disp where N_CompanyID=@p1 and ISNULL(x_EmpsID,'')='' or x_EmpsID like '%"+nEmpID+"%'" + Pattern  + Searchkey + " " + xSortBy;
            else
                sqlCommandText = "select top(" + nSizeperpage + ") X_ProjectCode,X_ProjectName,X_CustomerName,X_District,X_Name,N_StatusID,D_StartDate,N_ContractAmt,N_EstimateCost,AwardedBudget,ActualBudget,CommittedBudget,RemainingBudget,X_PO,N_Progress,N_CompanyID,N_Branchid,N_CustomerID,B_IsSaveDraft,B_Inactive,N_ProjectID,N_StageID,X_Stage,CONVERT(VARCHAR(10), D_EndDate,111) as D_EndDate,N_LastActionID,X_ClosingRemarks,x_TaskSummery from vw_InvProjectDashBoard_Disp where N_CompanyID=@p1 and ISNULL(x_EmpsID,'')='' or x_EmpsID like '%"+nEmpID+"%'" + Pattern + Searchkey + " and N_ProjectID not in (select top(" + Count + ") N_ProjectID from vw_InvProjectDashBoard_Disp where N_CompanyID=@p1 and ISNULL(x_EmpsID,'')='' or x_EmpsID like '%"+nEmpID+"%'" + Pattern + Searchkey + xSortBy + " ) " + xSortBy;
            Params.Add("@p1", nCompanyId);
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string sqlCommandCount = "select count(1) as N_Count  from vw_InvProjectDashBoard_Disp where N_CompanyID=@p1 ";
