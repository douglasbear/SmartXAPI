    ...
    public class Inv_BalanceAdjustment : ControllerBase
    {
        ...
        public ActionResult GetBalanceListDetails(int N_PartyType, string N_TransType, int nFnYearId, string X_ReceiptNo, bool bAllBranchData, int nBranchID, int nFormID, int nSalesOrderId)
        {
            ...
            if (bAllBranchData == true)
            {
                ...
                if (N_PartyType == 1)
                    Mastersql = "Select Inv_Customer.*,Inv_BalanceAdjustmentMaster.*,Inv_CustomerProjects.X_ProjectName,Inv_CustomerProjects.X_ProjectCode,Acc_CurrencyMaster.N_Decimal from Inv_BalanceAdjustmentMaster Inner Join  Inv_Customer ON Inv_BalanceAdjustmentMaster.N_PartyID = Inv_Customer.N_CustomerID And Inv_BalanceAdjustmentMaster.N_CompanyID = Inv_Customer.N_CompanyID and Inv_BalanceAdjustmentMaster.N_FnYearID=Inv_Customer.N_FnYearID  Left Outer Join "+
                     "Acc_CurrencyMaster ON Inv_Customer.N_CompanyID = Acc_CurrencyMaster.N_CompanyID AND Inv_Customer.N_CurrencyID = Acc_CurrencyMaster.N_CurrencyID Left Outer Join Inv_CustomerProjects ON Inv_BalanceAdjustmentMaster.N_ProjectID = Inv_CustomerProjects.N_ProjectID And Inv_BalanceAdjustmentMaster.N_CompanyID = Inv_CustomerProjects.N_CompanyID Where  Inv_BalanceAdjustmentMaster.N_CompanyID=@p1 and Inv_BalanceAdjustmentMaster.N_TransType =@p2 and Inv_BalanceAdjustmentMaster.N_FnYearID=@p3 and Inv_BalanceAdjustmentMaster.X_VoucherNo=@p4 and Inv_BalanceAdjustmentMaster.N_PartyType=@p5";
