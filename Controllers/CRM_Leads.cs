...
namespace SmartxAPI.Controllers
{
    ...
    public class CRM_Leads : ControllerBase
    {
        ...
        public ActionResult LeadList(int nFnYearId, int nPage, int nSizeperpage, string xSearchkey, string xSortBy)
        {
            ...
            if (Count == 0)
                sqlCommandText = "select top(" + nSizeperpage + ") * from vw_CRMLeads where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 " + Searchkey + " " + xSortBy;
            else
                sqlCommandText = "select top(" + nSizeperpage + ") * from vw_CRMLeads where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 " + Searchkey + " and N_LeadID not in (select top(" + Count + ") N_LeadID from vw_CRMLeads where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 " + xSortBy + " ) " + xSortBy;
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    sqlCommandCount = "select count(*) as N_Count  from vw_CRMLeads where N_CompanyID=@p1 and Left(X_Pattern,Len(@p2))=@p2 ";
