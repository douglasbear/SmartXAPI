...

{
    ...
    public class Inv_SalesInvoice : ControllerBase
    {
        ...
        public ActionResult GetSalesInvoiceList(int? nCompanyId, int nFnYearId, int nPage, int nSizeperpage)
        {
            ...
            string OrderBy = "order by D_SalesDate desc";
            if (Count == 0)
                sqlCommandText = "select top(" + nSizeperpage + ") * from vw_InvSalesInvoiceNo_Search where N_CompanyID=@p1 and N_FnYearID=@p2 " + OrderBy + "";
            else
                sqlCommandText = "select top(" + nSizeperpage + ") * from vw_InvSalesInvoiceNo_Search where N_CompanyID=@p1 and N_FnYearID=@p2 and N_SalesID not in (select top(" + Count + ") N_SalesID from vw_InvSalesInvoiceNo_Search where N_CompanyID=@p1 and N_FnYearID=@p2) " + OrderBy + "";
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    sqlCommandCount = "select count(*) as N_Count  from vw_InvSalesInvoiceNo_Search where N_CompanyID=@p1 and N_FnYearID=@p2";
