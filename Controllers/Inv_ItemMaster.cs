    ...
    public class Inv_ItemMaster : ControllerBase
    {
        ...
        
        {
            ...
            if (query != "" && query != null)
            {
                if (partNoEnable)
                {
                    qry = " and (Description like @query or [vw_InvItem_Search_Products].[Part No] like @query or vw_InvItem_Search_Products.X_Barcode like @query or [vw_InvItem_Search_Products].[itemCode] like @query) ";
                    Params.Add("@query", "%" + query + "%");
                }
                else
                {
