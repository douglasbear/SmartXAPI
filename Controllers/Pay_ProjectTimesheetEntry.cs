...
namespace SmartxAPI.Controllers
{
    ...
    public class Pay_ProjectTimesheetEntry : ControllerBase
    {
        ...
        public ActionResult GetAllProjectTimesheet(int nComapanyId, int nFnYearId, int nPage, int nSizeperpage, string xSearchkey, string xSortBy)
        {
            ...
            if (Count == 0)
                sqlCommandText = "select top(" + nSizeperpage + ") * from vw_Prj_TimeSheetMaster where N_CompanyID=@p1 and N_UserID=@userID  " + Searchkey + " " + xSortBy;
            else
                sqlCommandText = "select top(" + nSizeperpage + ") * from vw_Prj_TimeSheetMaster where  N_CompanyID=@p1 and N_UserID=@userID " + Searchkey + " and N_PrjTimeSheetID not in (select top(" + Count + ") N_PrjTimeSheetID from vw_Prj_TimeSheetMaster where N_CompanyID=@p1 and N_UserID=@userID  " + xSearchkey + xSortBy + " ) " + xSortBy;
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    sqlCommandCount = "select count(1) as N_Count from vw_Prj_TimeSheetMaster where  N_CompanyID=@p1 and N_UserID=@userID " + Searchkey + " ";
